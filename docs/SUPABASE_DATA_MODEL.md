...
| **athlete_performance** | **Primary ledger for class WOD completion** when programming is shared: one row per athlete per completed piece, linking `contact_id`, optional `programming_id`, optional `programming_line_item_id`, optional **`segment_group_id`** (multi-part block total with `programming_id` null), scores/PR flags. Views: **`pli_movement_components`**, **`segment_group_results`**. |
