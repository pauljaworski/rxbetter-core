...
| **athlete_performance** | **Primary ledger for class WOD completion** when programming is shared: one row per athlete per completed piece, linking `contact_id`, optional `programming_id`, optional `programming_line_item_id`, scores/PR flags. If `programming_line_item_id` is set, RLS requires it to belong to the same `programming_id` and to be shared or owned by that same athlete. Also used for custom workouts tied to gym or personal `programming`; personal off-tenant results are athlete-owned on read. |
...
Further migrations: `20260504120000_hybrid_completion_athlete_custom_programming.sql`, `20260505131000_enforce_performance_line_item_parent.sql`.

## Personas & RLS (summary)

Row-level security is defined in migrations (`20260503120000_persona_rls_and_library_scope.sql`, `20260504120000_hybrid_completion_athlete_custom_programming.sql`, `20260505131000_enforce_performance_line_item_parent.sql`). In short:

- **Athlete** — Updates own `contact` / `profiles`; may insert/update own `athlete_benchmark_summary`; inserts **`athlete_performance`** for class completion when the referenced programming/line item are consistent and in scope; may insert/update **own** `programming` / `programming_line_item` when `source = 'athlete_custom'` and `created_by_contact_id` matches; may update **only result fields** on **individualized** `programming_line_item` (`contact_id` = self), not on **shared** (`contact_id` null) lines.
