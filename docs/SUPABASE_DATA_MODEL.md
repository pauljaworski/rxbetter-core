...
| **programming_intake_stage** | Staging for plain-text WOD intake: `raw_text`, `parsed_payload` (JSON), `parser_mode`, `status` (`staged` / `committed` / `rejected`), optional `committed_programming_id`. Audit trail before commit to `programming`. |
