...
2. After insert into **`auth.users`**, the trigger **`on_auth_user_created`** runs **`public.handle_new_user`**, which links exactly one unclaimed **`public.contact`** with the same email when safe, otherwise inserts a new **`contact`** (`user_id`, `email`, names from metadata). It then creates **`public.profiles`** (`id` = auth uid, `contact_id`, `display_name` derived from metadata or email local-part).
...
Migrations: `20260510120000_auth_signup_contact_profile.sql`, then `20260513130227_link_existing_contact_on_signup.sql`.
