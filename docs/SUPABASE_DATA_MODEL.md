...
| **programming_line_item** | Prescribed sets/reps/movements within one `programming` row; optional **`contact_id`** for individualized lines. **`line_item_kind`** (`strength_set`, `complex_set`, `metcon_movement`, …) and **`movement_components`** JSONB for complexes (e.g. 2 Clean Pull + 1 PC). **`benchmark_definition_id`** ties % prescriptions to a rep max (1RM, 2RM, etc.). **`movement_label`** when **`benchmark_type_id`** is null (custom gym movement). **Class / shared prescription:** `contact_id` null — athletes insert **`athlete_performance`** on the PLI (strength/complex), on **`programming_id`** (single-segment metcon), or on **`segment_group_id`** (multi-part total). |
| **athlete_performance** | **Primary ledger for class WOD completion** when programming is shared: one row per athlete per completed piece, linking `contact_id`, optional `programming_id`, optional `programming_line_item_id`, optional **`segment_group_id`** (multi-part block total with `programming_id` null), `score`, `result_value`, optional **`score_meta`** JSONB. Views: **`pli_movement_components`**, **`segment_group_results`**. |
| **athlete_segment_completion** | Per-athlete segment/block completion for Today journey UX: `programming_id` (single segment) or `segment_group_id` (multi-part total), keyed by `performance_date`. Auto-written when all PLI lifts or segment/group score is logged. |
| **athlete_day_completion_summary** | View: daily count of completed segments/blocks per athlete. |
| **programming_movement_exposure** | View: movement rows per published programming for reporting (includes complex components via `pli_movement_components`). |
...
2. After insert into **`auth.users`**, the trigger **`on_auth_user_created`** runs **`public.handle_new_user`**. If exactly one unclaimed **`public.contact`** already has the same normalized email, the trigger links that contact to the auth user so pre-created memberships/subscriptions stay attached; otherwise it inserts a new contact. It then inserts **`public.profiles`** (`id` = auth uid, `contact_id`, `display_name` derived from metadata or email local-part).
...
Migrations: `20260510120000_auth_signup_contact_profile.sql`, `20260515130500_link_signup_to_existing_contact.sql`.
