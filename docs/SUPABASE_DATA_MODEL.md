...
| **benchmark_type** | Movement/benchmark category. **`gym_id` NULL** = platform catalog (all gyms; admin write). **`gym_id` set** = gym-owned custom movement (visible only to that gym; programmers create via `ensure_gym_benchmark_type`). |
...
| **programming_line_item** | Prescribed sets/reps/movements within one `programming` row; optional **`contact_id`** for individualized lines. **`line_item_kind`** (`strength_set`, `complex_set`, `metcon_movement`, `rest`, …) and **`movement_components`** JSONB for multi-move sets (olympic complexes or strength circuits with optional units / `rest_after_sec`). **`rest_sec`**: rest after the set / between multi-move sets. **`benchmark_definition_id`** ties % prescriptions to a rep max (1RM, 2RM, etc.). **`movement_label`** when **`benchmark_type_id`** is null (legacy); new customs are written to gym-scoped **`benchmark_type`**. **Class / shared prescription:** `contact_id` null — athletes insert **`athlete_performance`** on the PLI (strength/complex), on **`programming_id`** (single-segment metcon), or on **`segment_group_id`** (multi-part total). |
