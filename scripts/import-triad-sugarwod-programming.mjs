...
sql.push('');
sql.push(`delete from public.athlete_performance
where programming_id in (
  select p.id
  from public.programming p
  join public.gym g on g.id = p.gym_id
  where g.name ilike 'Triad Training'
    and p.wod_date between ${sqlStr(minDate)} and ${sqlStr(maxDate)}
    and p.source = 'gym'
    and p.id::text like 'e5000000%'
);`);
sql.push(`delete from public.programming_library_assignment
where programming_id in (
  select p.id
  from public.programming p
  join public.gym g on g.id = p.gym_id
...
sql.push(`delete from public.programming_line_item
where programming_id in (
  select p.id
  from public.programming p
  join public.gym g on g.id = p.gym_id
...
sql.push(`delete from public.programming
where id in (
  select p.id
  from public.programming p
  join public.gym g on g.id = p.gym_id
