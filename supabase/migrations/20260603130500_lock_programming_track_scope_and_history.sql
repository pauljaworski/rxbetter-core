-- Lock programming write/delete paths to every affected track and preserve
-- athlete history when staff removes published segments.
...
-- ---------------------------------------------------------------------------
-- Parent and line-item update guards
-- ---------------------------------------------------------------------------
...
begin
  ...
  v_coach_only :=
    (to_jsonb(new) - 'coaches_notes' - 'updated_at')
    = (to_jsonb(old) - 'coaches_notes' - 'updated_at');

  if v_coach_only then
    v_coach_ok :=
  ...
  if old.source = 'gym'
     and not public.staff_can_manage_programming_tracks(old.id)
  then
    raise exception 'Insufficient privileges to update programming';
  end if;

  if public.staff_can_manage_programming_library(v_gym, v_lib) then
    return new;
  end if;
...
begin
  ...
  if v_source = 'gym'
     and public.staff_can_manage_programming_tracks(new.programming_id)
  then
    return new;
  end if;
