...
comment on function public.seed_triad_member is
  'POC: provision Triad Training athlete with CrossFit + Hyrox track access for one year. Creates auth user with a random unusable password when email provided; use Supabase password recovery/invite to grant access.';
