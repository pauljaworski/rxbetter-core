...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const availablePersonas = useMemo<Persona[]>(() => {
    ...
    if (roles.has("athlete")) personas.push("athlete");
    if (roles.has("coach")) personas.push("coach");
    if (roles.has("programmer")) personas.push("programmer");
    if (roles.has("admin")) personas.push("admin");
    return personas.length ? personas : ["athlete"];
  }, [activeMembership?.gym_id, activeMembership?.roles?.join("|")]);
