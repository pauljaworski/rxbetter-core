...
function mapWodsFromRows(
  ...
  return progs.map((p) => {
    ...
    const libIds =
      assignmentIds.length > 0
        ...
        : p.program_library_id
          ? [p.program_library_id]
          : [];
    const mappedItems = expandComplexSetLineItems(
      items
    ...
    return {
      ...
      items: mappedItems,
      // Copy/duplicate flows mark items new — no baseline deletes on first save.
      _knownLineItemIds: markNew
        ? []
        : mappedItems.map((it) => it.id).filter((id): id is string => Boolean(id)),
    };
  });
}
