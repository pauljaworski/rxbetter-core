...
const athletePerf: NavItem[] = [
  ...
  { to: "/my-programming", label: "My Programming", icon: PenLine },
...
export function RxSidebar({ className }: { className?: string }) {
  ...
  staffMenu.push({ to: "/my-programming", label: "My Programming", icon: PenLine });
  ...
  return (
    <Sidebar collapsible="icon" className={cn("border-r border-sidebar-border", className)}>
      <SidebarHeader className="border-b border-sidebar-border px-3 py-3 pr-12">
              ...
              <SidebarGroupContent>
                <SidebarMenu>
                  {athletePerf.map((item) => (
                    <SidebarMenuItem key={item.to}>
                      <SidebarMenuButton asChild>
                        <NavLink to={item.to} className={linkClass} onClick={closeMobileMenu}>
                          <item.icon className="h-4 w-4 shrink-0" />
                          {!collapsed && <span>{item.label}</span>}
                        </NavLink>
                      </SidebarMenuButton>
